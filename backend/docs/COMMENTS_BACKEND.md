...
3. **Прогрес читання** (після кроку 2; власник/творець книги — пропуск):
   - **книга:** `UserChapterProgress` з `chapter__book=book`, `is_read=True` (≥1 розділ); інакше 403 `detail`: «Щоб залишити коментар, потрібно прочитати хоча б один розділ».
   - **глава:** `is_read=True` для **цієї** глави; інакше 403 `detail`: «Щоб залишити коментар, потрібно прочитати цей розділ».
   - Деталі: **READING_PROGRESS_BACKEND.md** §6.
4. Перевірка авторизації: якщо не `request.user.is_authenticated` — 401.
5. Валідація даних сериалізатором: `text` (trim, довжина 3–1000), опційно `parent` (id батьківського коментаря). При помилках — 400.
6. `perform_create`: у `validated_data` підставляються `user` і `book`/`chapter`; збереження в БД.
7. Після успішного створення (зокрема відповіді) викликається **`record_comment_created`** для відповідної книги — оновлення **BookAnalytics** / **DailyAnalytics** для зважених метрик (ТОП тощо).
8. Відповідь — 201 і тіло створеного коментаря (сериалізатор).
...
**Останнє оновлення:** 2026-05-24 (перевірка `UserChapterProgress.is_read` при create; аналітика — 2026-03-21).
