...
| **Вивід балансу** | **Нові** (pending/awaiting_review) / схвалені / у batch / відправлені / завершені / скасовані / відхилені заявки; пакети batch для Wise (CSV); усі заявки (внизу) |
...
- **Нові заявки на виплату** — proxy-модель `NewPayoutRequest`, показує лише `pending` і `awaiting_review`. Перший пункт розділу, основна робоча вкладка.
- **Схвалені** — `approved` (після схвалення, до batch).
- **У batch** — `in_batch` (після створення batch CSV, до відправки в Wise).
...
- **Скасовані** — `cancelled`.
- **Відхилені Wise** — `failed`.
- **Пакети batch для Wise (CSV)** — зібрані групи; «Імпорт CSV» → Wise повертає статус COMPLETED → заявка в «Завершені».
- **Усі заявки на виплату** — повний список усіх заявок (незалежно від статусу); масові дії (схвалити, оновити курс, batch CSV, скасувати тощо). Останній пункт розділу.

Масові дії у заявках: **Схвалити** (авто-курс Wise API), **Оновити курс** (Wise API), **Створити batch CSV** (блокує якщо курс не встановлено), **Позначити batch відправленим**, **Позначити виплаченим**, **Скасувати з поверненням** (працює і для in_batch). При видаленні (delete) — модалка підтвердження українською: кошти НЕ повертаються.

Поля `exchange_rate`, `amount_net`, `withholding_tax_rate`, `withholding_tax_amount` — редаговані адміністратором.
