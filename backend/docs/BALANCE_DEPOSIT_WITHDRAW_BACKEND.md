...
## Сервісний збір при поповненні через Stripe

При поповненні через Stripe Checkout до суми додається сервісний збір: **`SERVICE_FEE_PERCENT`** % + **`SERVICE_FEE_FIXED_CZK`** CZK (конвертується в UAH за курсом Wise API). Збір включається в суму, яку Stripe списує з картки, але **не зараховується на баланс** — баланс поповнюється на `amount_coins` (оригінальна сума без збору). Детальний опис: `STRIPE_CHECKOUT_PAYMENTS_BACKEND.md`, розділ 5.3.
