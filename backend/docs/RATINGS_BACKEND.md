...
**Прогрес читання:** після перевірки `rate_permission` — для користувачів, які **не** власник/творець книги, потрібен хоча б один розділ з `UserChapterProgress.is_read=True` у цій книзі. Інакше **403** з `error`: «Щоб оцінити книгу, потрібно прочитати хоча б один розділ». Деталі: **READING_PROGRESS_BACKEND.md** §6.
...
**Останнє оновлення документа:** 2026-06-21 — без змін API рейтингу; вимога прочитання глави (§4.2) працює через оновлений прогрес читання (max-захист, формула часу 55 с/1000 символів) — деталі в READING_PROGRESS_BACKEND.md.
