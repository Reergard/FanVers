...
**Призначення:** Кастомний компонент вибору дати — текстовий input (формат `дд.мм.рррр`) + випадаючий мини-календар.

**Файли:**
- `shared/DatePickerField/DatePickerField.tsx`
- `shared/DatePickerField/DatePickerField.module.css`

**Особливості:**
- Текстовий input: фільтрує все, крім цифр і крапок; валідація при `blur` (не посимвольно)
- Dropdown-календар: з'являється при фокусі або кліку на іконку; закривається по `Escape`, кліку ззовні або вибору дати
- Українська локаль (назви місяців, дні тижня Пн–Нд)
- Підтримка `min` / `max` дат: заблоковані дні відображаються сірим, стрілки навігації вимикаються
- Позиціювання: адаптивне (вгору або вниз залежно від вільного місця)
- Стиль: cyan рамки (#00d2ff), оранжевий (#f58807) заголовок місяця і маркер «сьогодні», BadScript шрифт
- Портал (`createPortal`) для коректного z-index

**Місця використання:**
- `catalog/settings/Advertising.tsx` — поля «дата початку» / «дата закінчення» реклами
...
**Призначення:** Спільний скролер для каруселей з `BookCard`: стрілки, зірки-пагінація, сенсорний свайп (JS drag, axis-lock, snap), перетягування мишкою на ПК (відкладений старт drag для збереження кліків по `Link`). Опційна **автопрокрутка** (лише реклама).
