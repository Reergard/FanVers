...
- Условие «прочитано» на бекенде (после max-защиты): `reading_time >= min_reading_time` **И** (`scroll_position >= 90` **ИЛИ** `min_reading_time == 0`) → `is_read` (нужно для комментария к главе и рейтинга книги).
