...
## Потоки
- **Bootstrap**: `fetchCsrfToken → refreshSessionSilent → markBootstrapped`.
...
- `auth/service.ts`: login/register, `refreshSessionSilent` (cooldown + exp‑порог), `refreshSession`, `logoutSession`, `authStatus`.
...
- `auth/bootstrap.ts`: `bootstrapAuth` и `attachAuthAutoRefresh`.
- `auth/store.ts`: `csrfToken` и `bootstrapped`.
