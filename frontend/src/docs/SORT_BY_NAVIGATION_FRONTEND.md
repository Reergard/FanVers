...
- Після зміни сорту скидається `visibleCount`.
- Додавання/видалення закладки через API оновлює на бекенді лічильники **аналітики** (ТОП тощо; окремий виклик з UI не потрібен). Див. `backend/docs/ANALYTICS_BOOKS_BACKEND.md`.
