...
- Якщо так: викликається `showSuccessAutoClose("Глава успішно завантажена")`, потім `navigate(location.pathname, { replace: true, state: {} })` — щоб при оновленні сторінки state не лишався і модалка не показувалась повторно.
...
Підсумок: після успішного створення глави користувач потрапляє на сторінку книги, BookDetailRouter показує модалку «Глава успішно завантажена» без кнопок; вона сама зникає через 3 секунди.
