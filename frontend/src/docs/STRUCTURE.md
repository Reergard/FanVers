...
**Що це:** кастомний компонент вибору дати — текстовий input (`дд.мм.рррр`) + випадаючий мини-календар (портал).

**Особливості:**
- Ручний ввод з валідацією при `blur` (без посимвольної перевірки — вирішує баг нативного `<input type="date">`)
- Dropdown-календар з навігацією по місяцях, блокуванням мін/макс дат, маркером «сьогодні»
- Українська локаль (Січень–Грудень, Пн–Нд)
- `Escape` закриває, клік ззовні закриває, вибір дати закриває
- Адаптивне позиціювання (вгору або вниз)
- Стиль: cyan (#00d2ff) рамки/текст, оранжевий (#f58807) заголовок/маркер сьогодні, BadScript шрифт

**Використання:** `catalog/settings/Advertising.tsx` — поля дат реклами.
